...
export default function PrivateChatWindow({
  ...
  // Auto scroll to last message when privateMessages update
  useEffect(() => {
    ...
    return (
      <div className="flex-1 flex items-center justify-center text-gray-500 bg-white rounded-lg m-4">
        <div className="text-center p-6">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mx-auto text-indigo-300 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div className="flex-1 flex flex-col h-full bg-white rounded-lg m-4 shadow-sm border">
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
        {(privateMessages[activeChat] || []).map((m, i) => (
          ...
          >
            <div className={`inline-block max-w-xs lg:max-w-md xl:max-w-lg rounded-lg px-4 py-2 ${m.from === currentUser
              ? "bg-indigo-500 text-white rounded-br-none"
              : "bg-gray-200 text-gray-800 rounded-bl-none"
              }`}
            >
              <p>{m.text}</p>
              <span className={`text-xs block mt-1 ${m.from === currentUser ? "text-indigo-200" : "text-gray-500"}`}>
